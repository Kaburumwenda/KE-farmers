...
from POS.models import Order, OrderProduct
from PMS.models import Category, Comment
from UMS.forms import SignUpForm, UserUpdateForm, ProfileUpdateForm
...
def login_form(request):
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            # Redirect to a success page.
            return HttpResponseRedirect('/'+ settings.LANGUAGE_CODE1)
        else:
            messages.warning(request,"Login Error !! Username or Password is incorrect")
            return HttpResponseRedirect('/login')
    # Return an 'invalid login' error message.
...
def signup_form(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            data.image="users/images/userdefault.jpg"
            data.save()
